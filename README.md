...
> 重要说明：当前 `Node.js-visualization` 分支仅用于额外可视化实验和个人演示，不作为本 Java 课程作业提交内容。课程作业要求以 Java 为主体，正式提交请使用 `main` 分支中的 Java 控制台与 Swing GUI 版本。

## 1. 项目概述

这是一个 Java 课程作业项目，用于管理王者荣耀风格的数据：玩家、英雄、装备、战队和对战记录。系统支持控制台和 Swing GUI 两种 Java 入口，覆盖登录、查询、排行榜、管理员数据管理、JSON 保存和加载，以及基于英雄类型的装备推荐。
...
本分支新增 `visualization/` 子项目，用于把同一份 `data/game-data.json` 展示为网页仪表盘。该内容只是额外实验，不计入课程作业范围，也不作为作业运行方式：
...
- 管理员和教练超级账户不进入公开玩家检索
- Node.js 可视化仪表盘：总览图表、玩家筛选、战队详情、英雄推荐、装备评分和对战时间线（仅本分支额外实验，不属于作业提交内容）
