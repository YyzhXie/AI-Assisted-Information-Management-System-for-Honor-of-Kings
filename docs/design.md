...
系统为每名玩家自动选择其拥有英雄中基础战力最高的英雄。装备加成只取该英雄兼容装备中评分最高的前 3 件。每回合根据攻击能力、防御能力、地图节奏、团队士气、装备触发、暴击、闪避和随机波动计算双方伤害。若 5 回合后生命值相同，依次按战队基础战力、历史胜率和战队 ID 升序打破平局，保证单局模拟没有平局。
