...
- `CombatSimulator` 使用当前战队、玩家、英雄和装备数据生成模拟战报，但不创建 `MatchRecord`，也不修改玩家胜负场。
...
`GuiMain` 是 Swing 可视化入口，启动时优先读取 `data/game-data.json`，失败时回退 `DataInitializer` 内置数据。`VisualizationFrame` 使用页签组织六项可视化功能：玩家查询、战队概览、英雄详情、排行榜、对战模拟和编辑信息。玩家、战队和英雄页签采用“搜索框 + 列表 + 详情面板”的结构；排行榜页签使用表格展示名次、玩家 ID、昵称、等级、胜率和对战次数；对战模拟页签使用两个战队下拉框和只读报告区展示 5 回合模拟结果。GUI 窗口标题和页内主标题统一显示为“王者荣耀信息管理系统”，不在可视化窗口中显示 AI 相关词句、提示语或 Swing 可视化提示文案。
...
“编辑信息”页签使用 `CardLayout` 根据当前登录身份切换界面。未登录时显示居中提示“登录以查看信息。”；玩家登录后只显示自己的昵称和密码编辑表单；管理员或教练登录后显示通用数据维护界面，可按数据类型连续创建、修改或删除管理员、玩家、英雄、装备、战队和对战记录。登出会清空当前用户并强制切回未登录卡片，避免旧账号的编辑界面残留。

管理员 GUI 表单对业务 ID 使用自动编号和锁定显示：玩家、英雄、装备、战队和对战记录分别按当前最大编号生成下一位 `Pxxx`、`Hxxx`、`Exxx`、`Txxx`、`Mxxx`，保存时不允许手工改写 ID。玩家新建时用户名预填同玩家 ID，但仍可编辑；英雄和对战记录全部字段必填；战队名称必填，成员 ID 可为空，允许先建立空战队再由超级用户后续维护成员。保存前会校验所有非空引用是否存在，例如玩家的战队和英雄、英雄的兼容装备、战队成员、对战双方和英雄选择；非法引用会被集中提示并拒绝保存。

Swing GUI 属于正式 Java 作业内容。`Node.js-visualization` 分支仅作为额外实验，不作为作业提交内容。

## 对战模拟公式

对战模拟是加分演示功能，只生成预览报告，不写入 `MatchRecord`，不改变玩家胜负场、战队历史胜率、排行榜或 JSON 数据。控制台和 Swing GUI 使用同一个 `CombatSimulator` 服务。
...
+ defense * 0.24
+ playerLevel * 1.8
+ top3EquipmentRatingSum * 4.0
```

模拟严格按 5v5 成立。若战队可参赛成员少于 5 人，系统拒绝模拟；若战队人数超过 5 人，系统使用同一随机源随机抽取 5 名成员上场，以模拟真实比赛替补情况。系统为每名上场玩家自动选择其拥有英雄中基础战力最高的英雄。装备加成只取该英雄兼容装备中评分最高的前 3 件。每回合根据攻击能力、防御能力、地图节奏、团队士气、装备触发、暴击、闪避和随机波动计算双方伤害。若 5 回合后生命值相同，依次按战队基础战力、历史胜率和战队 ID 升序打破平局，保证单局模拟没有平局。
